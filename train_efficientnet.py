...
Entrenamiento preliminar EfficientNetV2S + CBAM
...
class EfficientNetCBAM(nn.Module):
    def __init__(self, num_classes=4):
        ...
        # CBAM en últimas 3 etapas (bloques 4, 5, 6)
        self._insert_cbam()
    ...
    def _insert_cbam(self):
        bloques_cbam = {4: 128, 5: 160, 6: 256}
        for idx, canales in bloques_cbam.items():
            bloque_orig = self.features[idx]
            self.features[idx] = nn.Sequential(bloque_orig, CBAM(canales))
...
def get_loaders():
    ...
    def remap(ds):
        old = ds.classes
        m   = {i: logica[c] for i, c in enumerate(old)}
...
def entrenar():
    ...
    model     = EfficientNetCBAM().to(DEVICE)
    ...
    # Descongelar solo los CBAM insertados
    for idx in [4, 5, 6]:
    ...
    for ep in range(EPOCAS_F1):
        ...
        print(f"  F1 Época {ep+1:2d}/{EPOCAS_F1} | "
    ...
    # ── FASE 2: fine-tuning ───────────────────────────────
    print(f"\n{'='*55}")
    print(f"FASE 2 — Fine-tuning (últimas 3 etapas + CBAM + cabezal)")
    print(f"{'='*55}")
    torch.cuda.empty_cache()

    # Descongelar últimas 3 etapas del backbone
    for idx in [4, 5, 6]:
    ...
    for ep in range(EPOCAS_F2):
        ...
        if vl_f1 > best_f1:
            ...
            patience += 1
        print(f"  F1 Época {ep+1:2d}/{EPOCAS_F2} | "
              f"Loss {tr_loss:.4f}/{vl_loss:.4f} | "
              f"Acc {tr_acc:.4f}/{vl_acc:.4f} | "
              f"F1 {vl_f1:.4f} | {elapsed:.1f}s")
